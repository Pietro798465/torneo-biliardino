document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- FUNZIONI UTILITY E RENDER (invariate) ---
    ...
    // Funzione helper per la password
    function executeAdminAction(confirmationMessage, action, requireConfirmation = true) {
        const password = prompt("Inserisci la password amministratore:");
        if (password === adminPassword) {
            if (!requireConfirmation || confirm(confirmationMessage)) {
                action();
            }
    ...
    generateRoundRobinBtn.addEventListener("click", () => {
        executeAdminAction("Sei sicuro di voler generare i gironi? Le partite esistenti verranno cancellate.", async () => {
