document.addEventListener('DOMContentLoaded', () => {
    ...
    const generateKnockoutMatches = async (isRandom) => {
        ...
        const message = isRandom ? "Generare le semifinali con sorteggio CASUALE tra le prime 4?" : "Generare le semifinali STANDARD (1ª vs 4ª e 2ª vs 3ª)?";
    ...
    function renderTeams(){teamsList.innerHTML="";localTeams.forEach(t=>{const l=document.createElement("div");l.className="team-item",l.innerHTML=`<input type="text" class="team-name-input" value="${t.name}" onchange="updateTeamName('${t.id}', this.value)"><span class="team-players">${photoHTML(t.player1)}${photoHTML(t.player2)} ${t.player1.name} & ${t.player2.name}</span>`,teamsList.appendChild(l)})}
    
    function renderRoundRobinMatches(){roundRobinMatchesDiv.innerHTML="";localRoundRobinMatches.forEach(e=>{const t=document.createElement("div");t.className="match-item",t.innerHTML=`<span>${photoHTML(e.teamA.player1)}${photoHTML(e.teamA.player2)}${e.teamA.name}</span><input type="number" value="${e.scoreA??""}" onchange="updateScore('${e.id}', 'A', this.value)"><span class="vs">vs</span><input type="number" value="${e.scoreB??""}" onchange="updateScore('${e.id}', 'B', this.value)"><span>${e.teamB.name}${photoHTML(e.teamB.player1)}${photoHTML(e.teamB.player2)}</span>`,roundRobinMatchesDiv.appendChild(t)})}
    
    function renderKnockoutBracket(){knockoutStageDiv.innerHTML="";if(0===localKnockoutMatches.length)return;const e=localKnockoutMatches.filter(e=>1===e.round).sort((e,t)=>e.matchIndex-t.matchIndex),t=e[0],l=e[1];let a='<div class="knockout-round"><h3>Semifinali</h3>';t&&(a+=createMatchupHTML(t)),l&&(a+=createMatchupHTML(l)),a+="</div>";const n=t&&null!==t.scoreA&&null!==t.scoreB?t.scoreA>t.scoreB?t.teamA:t.teamB:null,c=l&&null!==l.scoreA&&null!==l.scoreB?l.scoreA>l.scoreB?l.teamA:l.teamB:null;let o='<div class="knockout-round"><h3>Finale</h3>',r=localKnockoutMatches.find(e=>2===e.round);n&&c&&!r&&(r={round:2,matchIndex:0,teamA:n,teamB:c,scoreA:null,scoreB:null},db.collection("knockoutMatches").add(r).then(e=>db.collection("knockoutMatches").doc(e.id).update({id:e.id}))),o+=r?createMatchupHTML(r):createMatchupHTML({teamA:{name:"Da definire", player1:{}, player2:{}},teamB:{name:"Da definire", player1:{}, player2:{}}}),o+="</div>",knockoutStageDiv.innerHTML=a+o}
    
    function createMatchupHTML(e){const t=e.id||"",l=e.scoreA??"",a=e.scoreB??"",n=null!==e.scoreA&&l>a,c=null!==e.scoreB&&a>l;return`<div class="knockout-matchup"><div class="knockout-team team-a ${n?"winner":""}"><span class="team-name-knockout">${photoHTML(e.teamA.player1)}${photoHTML(e.teamA.player2)}${e.teamA.name}</span></div><input type="number" class="score-knockout" value="${l}" ${t?`onchange="updateKnockoutScore('${t}', 'A', this.value)"`:"disabled"}><span class="knockout-vs">vs</span><input type="number" class="score-knockout" value="${a}" ${t?`onchange="updateKnockoutScore('${t}', 'B', this.value)"`:"disabled"}><div class="knockout-team team-b ${c?"winner":""}"><span class="team-name-knockout">${e.teamB.name}${photoHTML(e.teamB.player1)}${photoHTML(e.teamB.player2)}</span></div></div>`}
