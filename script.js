document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderRoundRobinMatches() {
        ...
        localRoundRobinMatches.forEach(m => {
            ...
            div.innerHTML = `<div class="match-item-desktop"><div class="team-info team-a">${photoHTML(m.teamA.player1)}${photoHTML(m.teamA.player2)}<span>${m.teamA.name}</span></div><div class="score-inputs"><input type="number" class="score-input ${cA}" value="${sA}" onchange="updateScore('${m.id}','A',this.value)"><span class="vs">vs</span><input type="number" class="score-input ${cB}" value="${sB}" onchange="updateScore('${m.id}','B',this.value)"></div><div class="team-info team-b"><span>${m.teamB.name}</span>${photoHTML(m.teamB.player2)}${photoHTML(m.teamB.player1)}</div></div><div class="match-item-mobile"><div class="match-row"><div class="team-details">${photoHTML(m.teamA.player1)}${photoHTML(m.teamA.player2)}<span>${m.teamA.name}</span></div><input type="number" class="score-input ${cA}" value="${sA}" onchange="updateScore('${m.id}','A',this.value)"></div><div class="vs-mobile">vs</div><div class="match-row"><div class="team-details">${photoHTML(m.teamB.player1)}${photoHTML(m.teamB.player2)}<span>${m.teamB.name}</span></div><input type="number" class="score-input ${cB}" value="${sB}" onchange="updateScore('${m.id}','B',this.value)"></div></div>`;
            roundRobinMatchesDiv.appendChild(div);
        });
    }
    
    // NUOVA FUNZIONE RENDERKNOCKOUTBRACKET STABILE
    function renderKnockoutBracket() {
        ...
        // Controlla se le semifinali sono state giocate e se la finale non è ancora stata creata
        if (semifinals.length === 2 && semifinals.every(m => m.scoreA !== null && m.scoreB !== null)) {
            if (!finalMatch) {
                const winner1 = semifinals[0].scoreA > semifinals[0].scoreB ? semifinals[0].teamA : semifinals[0].teamB;
                const winner2 = semifinals[1].scoreA > semifinals[1].scoreB ? semifinals[1].teamA : semifinals[1].teamB;
    ...
    // NUOVA FUNZIONE CREATEMATCHUPHTML STABILE
    function createMatchupHTML(m) {
        ...
        return `<div class="match-item">${desktopHTML}${mobileHTML}</div>`;
    }
    ...
    function updateLiveLeaderboard(standings){const lb=document.getElementById("live-leaderboard"),topList=document.getElementById("top-teams-list"),bottomList=document.getElementById("bottom-teams-list");if(standings.length===0)return;if(window.innerWidth > 768 || lb.classList.contains('visible')) lb.style.display="block";topList.innerHTML="",bottomList.innerHTML="";const qz=4;standings.slice(0,qz).forEach((s,i)=>{topList.innerHTML+=`<li><span><span class="team-pos">${i+1}.</span> ${s.name} ${s.tieBreakerWin?'<span class="tie-breaker-star">*</span>':""}</span><span class="team-points">${s.punti} Pt</span></li>`}),standings.slice(qz).forEach((s,i)=>{bottomList.innerHTML+=`<li><span><span class="team-pos">${qz+i+1}.</span> ${s.name}</span><span class="team-points">${s.punti} Pt</span></li>`})}
